-- Redeem fully-discounted coupon orders atomically.
-- The API calls this only with service_role. Everything below runs in the
-- single transaction created for the RPC call.
...
begin
  ...
  v_expires_at := v_now + make_interval(months => p_period_months);
...
comment on function public.redeem_free_coupon_order(uuid, text, integer, text, text, integer) is
  'Atomically creates a paid zero-value order, updates entitlement, writes history, and increments coupon usage.';

revoke all on function public.redeem_free_coupon_order(uuid, text, integer, text, text, integer) from public;
grant execute on function public.redeem_free_coupon_order(uuid, text, integer, text, text, integer)
  to service_role;

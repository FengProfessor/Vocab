...
### Canonical rollout stopped at migration (2026-09-26)

- PR `#4` merged to `main` at exact SHA `7d9d45e7fffb89d38dd1e826b9b42a985325dde6`. Canonical production workflow run `36223006832` used that exact SHA.
- Quality job `108351662358`: **PASS**. Clean `npm ci`, actionlint, deployment script/migration syntax, 14 deployment safety tests, health-route ESLint, application build, baseline-reporting typecheck, and baseline-reporting lint all completed successfully.
- Migration job `108352271617`: **FAIL** at `Apply migrations`. The runner connected to production and successfully applied `20260710_security_hardening.sql`, `20260710_atomic_paid_order_confirmation.sql`, and `20260716_class_scale_db_perf.sql`. It then failed on `20260718_words_example_vi.sql` with PostgreSQL error `cannot remove parameter defaults from existing function`.
- Migration executed: **YES, PARTIAL**. The first three migration files committed successfully before the fourth file failed. Treat their effects as applied; the canonical history/transaction behavior must be inspected before any retry.
- Deploy/restart/health executed: **NO**. Deploy job `108352367793` was skipped by the workflow dependency gate. No manual SSH, migration, restart, deploy, or CI bypass was attempted.
- Public verification after failure: `https://lingopro.online/api/health` still returns HTTP 404, consistent with the old pre-P0 release. `origin/main` equals `7d9d45e7fffb89d38dd1e826b9b42a985325dde6`; final production SHA remains the prior release and was not changed by this run.
- Rollout status: **STOPPED** at the exact failing migration stage. Operator residual-risk acceptance and deferred Ubuntu reimage remain recorded; no further production action is authorized in this checkpoint.

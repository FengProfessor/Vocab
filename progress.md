...
## P0-ROLLOUT — Canonical SSH render repair (2026-09-26)

- Canonical run `36224587351` at main SHA `9aabd1de7d4b874f442a75f6db89a7fec3de60c8`: Quality **PASS**, Migration **PASS**, Deploy **FAIL** before fetch/build/activation with `bash: -c: line 24: syntax error near unexpected token ';'`. Restart and health did not run; production remained on the old release and public `/api/health` remained HTTP 404.
- Exact root cause: `appleboy/ssh-action@v1.0.3` uses drone-ssh `v1.7.3`. With `script_stop: true`, drone-ssh appends an exit-code guard after every non-continuation line. It inserted `DRONE_SSH_PREV_COMMAND_EXIT_CODE=$? ; if [ $DRONE_SSH_PREV_COMMAND_EXIT_CODE -ne 0 ]; then exit $DRONE_SSH_PREV_COMMAND_EXIT_CODE; fi;` as remote line 24, directly between `case "$EXPECTED_SHA" in` and its first pattern. Bash expected a case pattern and rejected the injected semicolon.
- Previous syntax checks parsed the original inline YAML body and standalone repo scripts. The action logs the original script before `script_stop` mutation, and no test reconstructed the final remote payload, so the invalid transformed shell was invisible.
- Repair: remove `script_stop`, pass `EXPECTED_SHA` and `CRON_SECRET` through the action environment, retain a minimal exact-SHA bootstrap, and move staging build/orchestration into versioned `deploy/run-deploy.sh`. `set -eu`/`set -Eeuo pipefail`, exact detached checkout, dirty-source rejection, no branch fallback, release marker, `activate-release.sh`, rollback behavior, `lingopro.service`, and stable-PID HTTP readiness remain enforced.
...
- Clean GitHub Ubuntu verification run `36227293376` **PASS**: Node `22.23.2` / npm `10.9.8`, clean `npm ci`, actionlint, all syntax checks, existing 14 deployment tests, 7 new runner tests, rendered SSH payload `bash -n`, `git diff --check`, health-route ESLint and production build. Typecheck produced exactly the known 10 Speaking errors and no new error.
- PR `#6` merged normally at main SHA `a12394628a6b0a0772ffdd98c4e39d780f1b3256`. Canonical run `36227460622`: Quality **PASS**; Migration **PASS**, with all seven migrations `SKIP already applied` and probes/done successful; exact-SHA fetch/checkout **PASS**; server `npm ci` **PASS**; server build **PASS**.
- Canonical Deploy then **FAIL** at activation: the first `sudo -n systemctl stop lingopro.service` returned `sudo: a password is required`. The failure trap attempted rollback and its stop command failed for the same reason. The initial stop never succeeded, so live `.next` and env had not been moved, no restart occurred, and production continued serving the old release. Public root remains HTTP 200 and `/api/health` remains HTTP 404. The candidate had already been copied into a temporary `.next-candidate.*` directory; no cleanup was logged.
- Final production SHA: **not deployed / unchanged old release**. P0 rollout is **STOPPED** at canonical `Deploy via SSH`; resolving noninteractive sudo authorization is a separate reviewed task. No manual SSH, restart, migration, file change, rollback, or deployment workaround was attempted.

### Sudo remediation and successful canonical rerun (2026-09-26)

- Tailscale read-only audit confirmed server `lingo-sever`, deploy user `ubuntu`, and `/usr/bin/systemctl`. A dedicated `/etc/sudoers.d/lingopro-deploy` was installed as `440 root:root`; `visudo -cf` returned `parsed OK`. NOPASSWD is restricted to the four exact commands used by activation: `stop`, `restart`, `is-active --quiet`, and `show -p MainPID --value` for `lingopro.service`. All four were validated with `sudo -n -l` without changing service state.
- Failed jobs for canonical run `36227460622` were rerun as attempt `2` at the unchanged rollout SHA `a12394628a6b0a0772ffdd98c4e39d780f1b3256`. The successful Quality and Migration jobs were retained; migration was not reapplied. Deploy job `108380632492` fetched and verified the exact SHA, completed server `npm ci` and build, stopped/restarted `lingopro.service`, then obtained HTTP 200 twice with stable PID `1034115`. The previous release was preserved by the activation script.
- Independent read-only verification after workflow success: service active, MainPID `1034115`, staging HEAD and live `.next/.release-commit` both equal `a12394628a6b0a0772ffdd98c4e39d780f1b3256`, localhost `/api/health` returned `{"status":"ok"}`, public root returned HTTP 200, and public `/api/health` returned HTTP 200 with the same body.
- Final result: canonical workflow `36227460622` attempt `2` **PASS** and production SHA is `a12394628a6b0a0772ffdd98c4e39d780f1b3256`. P0 production rollout is complete. No manual deploy, migration, service restart, bundle activation, or rollback was performed outside the workflow.
- Security follow-up: the Ubuntu password was accidentally displayed in an operator screenshot. No password value was stored in the repository or checkpoint; rotate that password promptly.

...
init_dir="$(mktemp -d)"
restore_log="$(mktemp)"
chmod 755 "$init_dir" "$sql_dir"
cleanup() {
  docker rm -f "$container" >/dev/null 2>&1 || true
  rm -f "$sql_dir/backup.sql" "$restore_log"
  rmdir "$sql_dir" "$init_dir" 2>/dev/null || true
}
trap cleanup EXIT
...
existing_schemas="$(docker exec "$container" psql -X -A -t -U postgres -d postgres -c \
...
if ! docker exec "$container" psql -X -q -v ON_ERROR_STOP=1 -v VERBOSITY=verbose \
  -U postgres -d postgres -f /restore/backup.sql >"$restore_log" 2>&1; then
...
objects_ok="$(docker exec "$container" psql -X -A -t -U postgres -d postgres -c \
...
functions_ok="$(docker exec "$container" psql -X -A -t -U postgres -d postgres -c \
...
for attempt in {1..30}; do
  if docker exec "$container" pg_isready -q -U postgres; then break; fi
  sleep 2
done
restart_ok="$(docker exec "$container" psql -X -A -t -U postgres -d postgres -c \

...
for (let b = 1; b <= 25; b++) {
  ...
  for (const f of files) {
    ...
    if (f.includes('Giao-An') || f.includes('giao-an')) {
      const raw = fs.readFileSync(fullPath, 'utf8');
      const cleaned = cleanGiaoAnContent(raw);
      if (cleaned !== raw) {
        fs.writeFileSync(fullPath, cleaned, 'utf8');
        console.log(`[Cleaned] ${pad}/${f}`);
        modifiedCount++;
      }
    } else if (f.includes('Phieu-Hoc-Tap') && f.endsWith('.html')) {
      const raw = fs.readFileSync(fullPath, 'utf8');
...
console.log(`\nSuccessfully cleaned ${modifiedCount} source files in buoi/ directory.`);

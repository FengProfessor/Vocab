...
function callCodexCLI(prompt: string, schemaPath: string): string {
  // Convert paths to use forward slashes to avoid Windows command shell parsing issues
  const safeSchemaPath = schemaPath.replace(/\\/g, '/');
  const tmpOutFile = path.join(DIR, `tmp-enrich-${Math.random().toString(36).substr(2, 9)}.json`).replace(/\\/g, '/');
  ...
  if (result.status !== 0) {
    throw new Error(`Codex CLI failed with exit code ${result.status}. Stderr: ${result.stderr || 'no stderr output'}`);
  }

  if (!existsSync(tmpOutFile)) {
    throw new Error('Codex CLI ran successfully but did not produce the expected output file');
  }
...
function validateExercise(ex: any): ex is Exercise {
  ...
  if (!ex.options.includes(ex.correct_answer)) return false;
...
async function main() {
  console.log('[enrich-exercises] Starting grammar exercise enrichment script...');
  ...
  console.log(`[enrich-exercises] Found ${targetFiles.length} lessons to check.`);
...
${JSON.stringify(existingBrief.slice(-30))}
...
4. Strictly return the output matching the provided JSON Schema. Do NOT add any extra text or wrapper outside of the JSON object.
`;
      ...
      while (!ok && retries > 0) {
        try {
          const rawResult = callCodexCLI(prompt, SCHEMA_PATH);
  ...
  console.log(`\n[enrich-exercises] Enrichment completed! Total new exercises generated: ${totalGenerated}`);
}

main().catch((err) => {
  console.error('[enrich-exercises] Fatal error in main loop:', err);
  process.exit(1);
});

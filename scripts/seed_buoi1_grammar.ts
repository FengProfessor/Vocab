...
async function seed() {
  console.log('Seeding Buổi 1 Grammar...');
  
  // 1. Insert/Get Topic
  const topicSlug = '25-chuyen-de-np-thpt';
  let { data: topic } = await supabase.from('grammar_topics').select('*').eq('slug', topicSlug).maybeSingle();
  
  if (!topic) {
    const { data: newTopic, error } = await supabase.from('grammar_topics').insert({
      slug: topicSlug,
      title: '25 Chuyên Đề Ngữ Pháp THPT',
      level: 'beginner',
      order_index: 100
    }).select().single();
    if (error) throw error;
    topic = newTopic;
    console.log('Created new topic:', topic.title);
  } else {
    console.log('Found topic:', topic.title);
  }

  // 2. Insert/Get Lesson
  const lessonTitle = 'Buổi 1 - Khởi động xương câu S-V-O';
  let { data: lesson } = await supabase.from('grammar_lessons').select('*').eq('topic_id', topic.id).eq('title', lessonTitle).maybeSingle();
  
  if (!lesson) {
    const { data: newLesson, error } = await supabase.from('grammar_lessons').insert({
      topic_id: topic.id,
      title: lessonTitle,
      order_index: 1,
      theory: 'Kịch bản giáo án đã dạy học sinh',
      exercises: exercises
    }).select().single();
    if (error) throw error;
    lesson = newLesson;
    console.log('Created new lesson:', lesson.title);
  } else {
    const { data: updatedLesson, error } = await supabase.from('grammar_lessons').update({
      exercises: exercises
    }).eq('id', lesson.id).select().single();
    if (error) throw error;
    lesson = updatedLesson;
    console.log('Updated existing lesson:', lesson.title);
  }

  console.log(`Success! Lesson ID is: ${lesson.id}`);
  console.log(`URL to test: /grammar?lessonId=${lesson.id}`);
}

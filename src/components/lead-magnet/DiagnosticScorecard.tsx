...
export default function DiagnosticScorecard({
  ...
  return (
    <div className="w-full bg-white border border-slate-200/90 rounded-3xl p-4 sm:p-7 shadow-xl shadow-slate-100/80 text-slate-900">
      {/* Header */}
      <div className="border-b border-slate-100 pb-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-200/80 text-indigo-700 text-xs font-semibold uppercase tracking-wider mb-2 shadow-xs">
            ...
            <p className="text-xs sm:text-sm text-slate-600 mt-1 leading-relaxed">
              Chấm điểm từ 0 – 2 cho từng bẫy để đo lường độ nhạy phản xạ và phát hiện chính xác lý do khiến bạn dậm chân tại chỗ.
            </p>
          </div>

          {/* Quick preset pills */}
          <div className="flex flex-wrap items-center gap-1.5 self-start sm:self-auto">
            <span className="text-[11px] text-slate-500 mr-1 hidden sm:inline font-medium">Thử nhanh:</span>
            <button
              type="button"
              onClick={() => handleQuickPreset('beginner')}
              className="px-2.5 py-1 text-[11px] rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-200 transition cursor-pointer font-medium"
            >
              Mất gốc (&lt;15đ)
            </button>
            <button
              ...
              className="px-2.5 py-1 text-[11px] rounded-lg bg-amber-50 text-amber-800 border border-amber-200 hover:bg-amber-100 transition cursor-pointer font-medium"
            >
              Kẹt 600-750 (18đ)
            </button>
            <button
              ...
              className="px-2.5 py-1 text-[11px] rounded-lg bg-emerald-50 text-emerald-800 border border-emerald-200 hover:bg-emerald-100 transition cursor-pointer font-medium"
            >
              Master 495 (30đ)
            </button>
            <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
          {/* Total */}
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/90 flex flex-col justify-between shadow-xs">
            <span className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold">Tổng điểm phản xạ</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-2xl sm:text-3xl font-mono font-black text-indigo-700">
                {totalScore}
              </span>
              <span className="text-xs text-slate-400 font-medium">/ 30</span>
            </div>
            <div className="text-[10px] text-slate-500 mt-1 font-medium">
              Đã đánh giá {answeredCount}/15 bẫy
            </div>
          </div>

          {/* Block 1 */}
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/90 flex flex-col justify-between shadow-xs">
            <span className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold">Part 1 (Tranh ảnh)</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-xl sm:text-2xl font-mono font-bold text-sky-700">{block1Score}</span>
          ...
          {/* Block 2 */}
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/90 flex flex-col justify-between shadow-xs">
            <span className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold">Part 2 (Hỏi - Đáp)</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-xl sm:text-2xl font-mono font-bold text-amber-700">{block2Score}</span>
              <span className="text-xs text-slate-400 font-medium">/ 10</span>
            </div>
            <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
              <div
          ...
          {/* Block 3 */}
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/90 flex flex-col justify-between shadow-xs">
            <span className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold">Part 3 &amp; 4 (Hội thoại)</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-xl sm:text-2xl font-mono font-bold text-purple-700">{block3Score}</span>
              <span className="text-xs text-slate-400 font-medium">/ 10</span>
            </div>
            <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
              <div
      ...
      {/* Tabs for Blocks */}
      <div className="flex items-center gap-2 border-b border-slate-200 pt-4 pb-1 overflow-x-auto text-xs sm:text-sm font-semibold">
        <button
          type="button"
        ...
        {filteredDimensions.map((dim: DimensionItem) => {
          ...
          return (
            <div key={dim.id} className="py-4 first:pt-2 last:pb-2">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                >
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <h3 className="font-bold text-sm sm:text-base text-slate-900 hover:text-indigo-600 transition">
                      {dim.nameVi}
                    </h3>
                ...
                <div className="flex items-center gap-1.5 self-start sm:self-center flex-shrink-0">
                  {dim.auditQuestion.options.map((opt) => {
                    ...
                    let activeStyles = 'bg-slate-100 text-slate-700 hover:bg-slate-200 border-slate-200';
                    if (isSelected) {
                      if (opt.score === 0) activeStyles = 'bg-rose-100 text-rose-800 border-rose-300 font-bold ring-1 ring-rose-300 shadow-xs';
                      else if (opt.score === 1) activeStyles = 'bg-amber-100 text-amber-900 border-amber-300 font-bold ring-1 ring-amber-300 shadow-xs';
                      else activeStyles = 'bg-emerald-100 text-emerald-900 border-emerald-300 font-bold ring-1 ring-emerald-300 shadow-xs';
                    }
                  ...
                  <button
                    type="button"
                    onClick={() => setExpandedId(isExpanded ? null : dim.id)}
                    className="p-1 text-slate-400 hover:text-slate-700 transition ml-1 cursor-pointer"
              ...
              {/* Accordion Deep Dive */}
              {isExpanded && (
                <div className="mt-3 p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-3 text-xs sm:text-sm animate-in fade-in duration-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    <div className="space-y-1">
                      <span className="text-slate-500 font-semibold uppercase tracking-wider block">
                        Bản chất Bẫy ETS
                      </span>
                      <p className="text-slate-700 leading-relaxed">{dim.trapDetail}</p>
                    </div>
                    <div className="space-y-1">
                      <span className="text-slate-500 font-semibold uppercase tracking-wider block">
                        Dẫn chứng Thực tế
                      </span>
                      <p className="text-emerald-800 font-mono bg-emerald-50 p-2.5 rounded-lg border border-emerald-200">
                        {dim.examProof}
                      </p>
                    </div>
                  </div>

                  <div className="pt-2 border-t border-slate-200 flex items-center justify-between text-xs text-indigo-700">
      ...
      <div className="mt-6 p-5 sm:p-7 rounded-2xl bg-gradient-to-br from-indigo-50/40 via-white to-emerald-50/30 border-2 border-indigo-200/90 shadow-lg space-y-5">
        {answeredCount === 0 ? (
          <div className="p-4 rounded-2xl bg-indigo-50 border border-indigo-200 flex items-start gap-3 text-slate-700 text-xs sm:text-sm">
            <HelpCircle className="w-5 h-5 text-indigo-600 flex-shrink-0 mt-0.5" />
            <div>
              <h4 className="font-bold text-slate-900 text-sm">Chưa có kết quả chẩn đoán</h4>
              <p className="mt-1 text-slate-600">
                Hãy bấm chọn mức phản xạ (0đ: Bị bẫy / 1đ: Chậm / 2đ: Phản xạ) cho từng bẫy ở các tab bên trên, hoặc bấm <strong>&quot;Thử nhanh&quot;</strong> ở góc phải trên để xem kịch bản mẫu.
              </p>
            </div>
          </div>
        ) : (
          <>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200/80 pb-4">
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-3xl sm:text-4xl font-black font-mono text-slate-900">
                    {totalScore} <span className="text-base text-slate-400 font-normal">/ 30 Điểm</span>
                  </span>
              ...
              <div className="text-left sm:text-right">
                <h4 className="text-base sm:text-lg font-bold text-slate-900">{currentTier.title}</h4>
            ...
            <div className="space-y-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-700">
                {currentTier.recommendations.map((rec, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-xl bg-white border border-slate-200 shadow-xs flex items-start gap-2"
                  >
                    <span className="w-5 h-5 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200 flex items-center justify-center font-mono font-bold flex-shrink-0 text-[11px]">
                      {idx + 1}
                    </span>
                    <span>{rec}</span>
                  </div>
          ...
          >
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                className="w-full sm:w-1/3 px-3.5 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-900 placeholder:text-slate-400 text-xs sm:text-sm focus:outline-none focus:border-indigo-600 transition shadow-xs"
              />
              <input
                type="email"
                required
                ...
                placeholder="Nhập Email để nhận Ebook & Báo Cáo Chi Tiết"
                className="w-full sm:w-2/3 px-3.5 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-900 placeholder:text-slate-400 text-xs sm:text-sm focus:outline-none focus:border-indigo-600 transition shadow-xs"
              />
              <button
                type="submit"
                disabled={isSubmitting}
                className="px-5 py-2.5 rounded-xl font-bold text-xs sm:text-sm text-white bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 transition shadow-md shadow-emerald-500/20 flex items-center justify-center gap-2 whitespace-nowrap cursor-pointer disabled:opacity-50"
              >
                {isSubmitting ? (
                  <span>Đang lưu...</span>
                ) : (
          ...
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs sm:text-sm flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <a
                ...
                download
                className="px-4 py-2 rounded-xl bg-emerald-600 text-white font-bold text-xs hover:bg-emerald-700 transition flex items-center gap-1.5 whitespace-nowrap shadow-xs"
              >
              ...
              <a
                ...
                download
                className="px-3 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 text-xs hover:bg-slate-50 transition shadow-xs"

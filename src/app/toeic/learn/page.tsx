...
function ToeicLearnContent() {
  ...
  // Handle lesson selection & URL synchronization
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 antialiased font-sans flex flex-col justify-between">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1.5">
                <h1 className="text-lg sm:text-xl font-black tracking-tight uppercase text-slate-900 dark:text-white flex items-center gap-2">
                  <BookOpen className="h-5 w-5 text-amber-600 dark:text-amber-400" />
                  <span>Cẩm Nang Lý Thuyết & Chiến Thuật</span>
                </h1>
                <span className="hidden sm:inline-block font-mono text-[11px] font-bold px-2 py-0.5 rounded-xs bg-amber-100 dark:bg-amber-950/70 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-800">
                  [{stats.totalLessons} Chủ Điểm]
                </span>
              </div>
            </div>

            {/* Quick Actions & Completed Switcher */}
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={handleToggleCompleteLesson}
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xs border text-xs font-mono font-semibold transition-all cursor-pointer select-none ${
              ...
              >
                ...
                />
                <span>{isCurrentLessonCompleted ? 'Đã hoàn thành' : 'Đánh dấu đã học'}</span>
              </button>
              ...
              <Link
                ...
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xs bg-slate-900 text-white dark:bg-white dark:text-slate-900 font-mono text-xs font-bold hover:bg-slate-800 dark:hover:bg-slate-100 transition-colors shadow-xs"
              >
                <Play className="h-3 w-3 fill-current" />
                <span>{currentLesson.bridgeToPractice?.ctaText || `Luyện Part ${currentLesson.targetPart}`}</span>
              </Link>
            </div>
          </div>
        </div>

        {/* ── SEGMENTED NAVIGATION TAB STRIP ── */}
        <div className="border-t border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-950/60 p-2 sm:p-2.5">
          <div className="mx-auto max-w-7xl">
            <div
              role="tablist"
              aria-label="Điều hướng phân hệ TOEIC"
              className="flex p-1 rounded-sm bg-slate-200/80 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 gap-1 text-xs font-mono"
            >
              {/* Tab 1: Đề Thi Full Test */}
              <Link
                href="/toeic?tab=full_test"
                className="flex-1 py-1.5 sm:py-2 px-2.5 rounded-xs font-semibold flex items-center justify-center gap-1.5 transition text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/50 dark:hover:bg-slate-700/50"
              >
              ...
              <Link
                href="/toeic/learn"
                className="flex-1 py-1.5 sm:py-2 px-2.5 rounded-xs font-semibold flex items-center justify-center gap-1.5 transition bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-xs font-bold"
              >
      ...
      {/* ── 3. MOBILE STICKY BOTTOM CONTROL BAR ── */}
